...
$award = ['Грамота Вченої ради (№ протоколу ВР КПІ)', 'Почесна грамота Вченої ради (№ протоколу ВР КПІ)', 'Почесна відзнака Вченої ради (№ протоколу ВР КПІ)', 'почесне звання «Заслужений викладач КПІ» (№ протоколу ВР КПІ)', 'почесне звання «Заслужений професор КПІ» (№ протоколу ВР КПІ)', 'почесне звання «Заслужений науковець КПІ» (№ протоколу ВР КПІ)', 'почесне звання «Заслужений працівник КПІ» (№ протоколу ВР КПІ)', 'почесна відзнака «Видатний діяч КПІ» (№ протоколу ВР КПІ)', 'почесне звання «Почесний доктор КПІ» (№ протоколу ВР КПІ)', 'почесна відзнака «За служіння та відданість КПІ» (№ протоколу ВР КПІ)', 'почесна відзнака «За заслуги  перед КПІ» (№ протоколу ВР КПІ)'];
...
<form method="post" action="{{ $action }}">
    ...
    <div class="form-group">
        ...
        <select name="state_award" id="stateAward" class="form-control">
            @foreach ($stateAwards as $stateAwards)
                <option value="{{ $stateAwards }}" @if (isset($employee) && $employee->stateAwards == $stateAwards) selected @endif>
                    {{ $stateAwards }}
                </option>
            @endforeach
        </select>
